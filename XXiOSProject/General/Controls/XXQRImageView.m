...
#import "XXQRImageView.h"

@implementation XXQRImageView
#pragma mark - Init
- (instancetype)initWithQRImageForString:(NSString *)string
{
    if (self = [super init]) {
        ...
        self.image = [self createNonInterpolatedUIImageFormCIImage:cImage withSize:300.0f];
        
        self.image = [self imageBlackToTransparent:self.image withRed:90 andGreen:136 andBlue:231];
...
- (instancetype)initWithQRImageForString:(NSString *)string avatar:(UIImage *)avatar
{
    if (self = [super init]) {
        ...
        if (avatar)
        {
            UIImage *avatarImg = [self createRoundedRectImage:avatar withSize:CGSizeMake(60, 60) withRadius:10];
            
            self.image = [self addIconToQRCodeImage:self.image withIcon:avatarImg withIconSize:CGSizeMake(60, 60)];
        }
    }
    return self;
}
...
- (CIImage *)createQRForString:(NSString *)qrString {
    ...
    [qrFilter setValue:@"M" forKey:@"inputCorrectionLevel"]; //设置纠错等级越高；即识别越容易，值可设置为L(Low) |  M(Medium) | Q | H(High)
    // Send the image back
